...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        else if (strncmp(input, "\\sighup ", 8) == 0)
        {
            ...
            if (pid > 0)
            {
                ...
                {
                    printf("Сигнал SIGHUP отправлен процессу с PID %d\n", pid);
                }
                else
                {
                    perror("Ошибка при отправке сигнала SIGHUP");
                }
            }
            else
            {
        ...
        {
            char *env_var = input + 3;
            char *value = getenv(env_var + 1);
            if (value)
            {
                printf("%s=%s\n", env_var, value);
            }
